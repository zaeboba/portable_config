...
function start_crop(mode)
	...
	if mp.get_property_native("vid") == false and mode == "hard" then
		mp.osd_message("Для обрезки видео сначала отключите подсветку чёрных полей", 2)
		return
	end	
    ...
    if mode_maybe ~= 'soft' then
        ...
        if hwdec and hwdec ~= "no" and not string.find(hwdec, "copy") then
           ...
           mp.osd_message("Обрезка и удаление логотипа не работают в режиме HW+ декодирования")
           return
        end
    end
    mp.commandv('script-message-to', 'uosc', 'disable-elements', mp.get_script_name(), 'timeline,volume,controls,top_bar,speed')
	if opts.recursive == false and mode == "hard" then
		local vf_list = mp.get_property("vf")
		local start_pos = 0
		while string.find(vf_list, "crop=", start_pos) do
			vf_crop1 = string.find(vf_list, "crop=", start_pos)
			start_pos = vf_crop1 + 5	
			vf_crop2 = string.find(vf_list, ",", vf_crop1)
			if vf_crop2 == nil then vf_crop = string.sub(vf_list ,vf_crop1)
			else vf_crop = string.sub(vf_list ,vf_crop1, vf_crop2-1) end
			mp.commandv("vf", "remove", vf_crop)
		end
	end
    active = true
