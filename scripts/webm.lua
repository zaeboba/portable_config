...
local options = {
	...
	output_directory = [[]],
    -- Set encoding process priority (idle | belownormal | normal | abovenormal | high)
    -- Recommended: belownormal or normal
    priority = "normal",
	run_detached = false,
  ...
  -- gif
	...
	twopass = true,
	-- If set, applies the audio/video and lavfi-complex filters currently used on the playback to the encode.
...
do
  ...
  local _base_0 = {
    draw = function(self)
      ...
      local progressText = string.format("%.1f%%", progress)
      local window_w, window_h = mp.get_osd_size()
...
encode = function(region, startTime, endTime)
  ...
  local lavfi_complex = mp.get_property("lavfi-complex")
  if options.apply_current_filters and lavfi_complex and lavfi_complex ~= "" then
    append(command, { 
    "--lavfi-complex=" .. lavfi_complex
    })
  end
  local af = mp.get_property("af")
  if options.apply_current_filters and af and af ~= "" then
    append(command, { 
    "--af=" .. af
    })
  end
  ...
  for track_type, tracks in pairs(supported_active_tracks) do
    ...
    repeat
      ...
      if (options.apply_current_filters and lavfi_complex and lavfi_complex ~= "") or ("video" == _exp_0) then
        append(command, {
          "--vid=no"
        })
  ...
  if options.run_detached then
    ...
    local res = false
    if options.display_progress == "no" or not should_display_progress() then
    ...
    if res then
      ...
      message("Encode failed! Check the logs for details.\nTry also disable displaying encode progress and retry encoding")
      emit_event("encode-finished", "fail")
    end
...
do
  ...
  local _base_0 = {
    ...
    draw = function(self, ass, selected)
      if selected then
        ass:append("{\\1c&Hffbf7f&}" .. tostring(bold(self.displayText)) .. ": {\\1c}")
      else
    ...
    __init = function(self, callback)
      ...
      self.options = {
        ...
        },
        {
          ...
          Option("bool", "Apply Current Audio and Video Filters", options.apply_current_filters)
        },
        {
          "scale_height",
...
do
  ...
  local _base_0 = {
    ...
    draw = function(self)
      ...
      ass:append(tostring(bold('t:')) .. " toggle displaying encode progress (current is " .. tostring(options.display_progress) .. ")\\N")
    ...
    toggle = function(self)
      if options.display_progress == "no" or options.display_progress == "auto" then
        options.display_progress = "yes"
      else
        options.display_progress = "no"
      end
      if self.visible then
        self:clear()
        return self:draw()
