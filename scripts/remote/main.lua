...
-- В автоматическом режиме файлы удаляются при закрытии плеера.
-- В ручном режиме файлы удаляются либо при повторном нажатии F7, либо при закрытии MPV,
-- при этом удаляются только те файлы, которые существуют.
----------------------------------------------------------------
local function cleanup()
    ...
    for _, file in ipairs(files_to_remove) do
        if file_exists(file) then
            ...
            msg.debug("Файл не существует, пропускаем: " .. file)
        end
    end
end
...
--   - Если скрипт включён (active = true), он отключается:
--       - Останавливается сервер, убивается таймер обновления,
--       - и вызывается cleanup() для удаления временных файлов (с проверкой их существования).
...
--         - При повторном нажатии скрипт отключится (и временные файлы будут удалены сразу).
--         - Если горячая клавиша не была нажата, файлы удалятся автоматически при закрытии MPV.
--
