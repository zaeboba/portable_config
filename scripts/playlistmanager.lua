local settings = {
  ...
  --привязки навигационных клавиш принудительно переопределяются только при видимом плейлисте
  --если «no», то вы можете отображать плейлист любой из навигационных клавиш
  dynamic_binds = true,

  -- чтобы связать несколько клавиш, разделяйте их пробелом

  -- главная клавиша для отображения списка воспроизведения
  key_showplaylist = "",

  -- отображение списка воспроизведения при нажатой клавише
  ...
  key_reverseplaylist = "",
  key_loadfiles = "",
  key_saveplaylist = "CTRL+F12",

  --заменяет совпадения в именах файлов на основе расширения, которое ставится как пустая строка, чтобы ничего не заменять
  --правила замены выполняются в указанном порядке
  --ключ правила замены - это шаблон, а значение - это значение замены
  --использует :gsub('pattern', 'replace'), подробнее http://lua-users.org/wiki/StringLibraryTutorial
  --'all' будет соответствовать любому расширению или протоколу, если у него есть такое расширение
  --использует json и разбирает его в таблицу lua, чтобы иметь возможность поддерживать файл .conf
...
--END OF SAMPLE REPLACE ]=====]
  ...
  --json-массив типов файлов для поиска в каталоге
  loadfiles_filetypes = [[
    [
  ...
  ---загружать файлы при запуске, если в плейлисте 1 или более элементов
  loadfiles_on_start = false,
  -- загрузка файлов из рабочего каталога при запуске вхолостую
  loadfiles_on_idle_start = false,
  --всегда помещайте загруженные файлы после воспроизводимого файла
  loadfiles_always_append = false,

  --сортировать список воспроизведения при добавлении файлов в список воспроизведения
  sortplaylist_on_file_add = false,

  --метод сортировки по умолчанию, должен быть одним из: «name-asc», „name-desc“, „date-asc“, „date-desc“, „size-asc“, „size-desc“.
  default_sort = "name-asc",

  --"linux | windows | auto"
  system = "auto",

  --Используйте ~ для домашнего каталога. Оставьте пустым, чтобы использовать dir рядом с исполняемым файлом mpv
  playlist_savepath = "",
  
  --Оставить пустым, чтобы сохранить в той же папке, где находятся файлы
  playlist_local_savepath = "",
  
  --Префикс имени плейлиста, полезный для размещения файлов плейлиста в папке с сортировкой по умолчанию в алфавитном порядке
  playlist_name_prefix = "playlist_",

  -- постоянное имя файла для сохранения плейлиста. Обратите внимание, что оно будет переопределять существующий плейлист. Оставьте пустым для сгенерированного имени.
  playlist_save_filename = "",

  --автоматическое сохранение плейлиста после выгрузки текущего файла
...
function save_playlist(filename)
  ...
  if string.find(mp.get_property('path'), '://') then
	if settings.playlist_savepath == nil or settings.playlist_savepath == "" then
		savepath = mp.command_native({"expand-path", "~~exe_dir/"})
	else
		savepath = parse_home(settings.playlist_savepath)
		if savepath == nil then return end
	end
  else
    if settings.playlist_local_savepath ~= "" then
      savepath = parse_home(settings.playlist_local_savepath)
    else
      local current_file = mp.get_property('path')
	  savepath = utils.split_path(current_file)
    end
  end

  
  if utils.readdir(savepath) == nil then
	-- mp.msg.warn("Path " .. settings.playlist_savepath .. " don't exist. Saving in folder near mpv's executable")
	-- savepath = mp.get_property("working-directory")
    local windows_args = {'powershell', '-NoProfile', '-Command', 'mkdir', savepath} --create savepath if it doesn't exist
  ...
  if name == nil then
	...
	if string.find(path, '://') then --для файлов по сети не можем получить название папки		
		substring = mp.get_property('media-title')
		substring = string.gsub(substring, "/", " ")
		substring = string.gsub(substring, "\\", " ")
		substring = string.gsub(substring, ":", "")
		substring = string.gsub(substring, "*", "x")
		substring = string.gsub(substring, "?", "")
		substring = string.gsub(substring, '"', "`")
		substring = string.gsub(substring, '<', "{")
		substring = string.gsub(substring, '>', "}")
		substring = string.gsub(substring, '|', " ")
        substring = substring .. GetShadersInfo(substring)  -- для функционала сборки: автовключение шейдеров Anime4k по тегу скриптом AnimeAnyK
	else
		local second_last_backslash = string.find(path, "\\[^\\]*\\[^\\]*$")
		if second_last_backslash == nil then second_last_backslash = 0 end --корень диска
		local last_backslash = string.find(path, "\\[^\\]*$")

		substring = string.sub(path, second_last_backslash + 1, last_backslash - 1)
		substring = string.gsub(substring, ":", "")
        substring = substring .. GetShadersInfo(substring)
	end
    local pl_name = settings.playlist_name_prefix .. substring .. ".m3u"
    name = pl_name
  end
  ...
  if not file then
    ...
    msg.info(saved_msg)
    file:close()
    mp.commandv("script-message-to", "auto_save_state", "playlist-created", savepath, name) -- для функционала сборки: сразу же наблюдаем созданный плейлист скриптом auto-save-state
  end
end
...
mp.register_script_message("save-playlist", activate_playlist_save)
