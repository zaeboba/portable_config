...
function display_state()
	local msg
	if active then
		msg = "Subtitle pausing enabled: \nListening only [r Replay]\nActive mode(s): ("
			.. (pause_at_start and "Reading" or "")
			.. ((pause_at_start and pause_at_end) and " and " or "")
			.. (pause_at_end and "Listening" or "") .. ")"
...
mp.add_key_binding("Ctrl+r", "toggle-start", function()
	pause_at_start = not pause_at_start
	toggle()
end)

mp.add_key_binding("Ctrl+l", "toggle-end", function()
	pause_at_end = not pause_at_end
	toggle()
end)

mp.add_key_binding("Ctrl+Alt+r", "skip-next", function() skip_next = true end)

mp.add_key_binding("r", "replay", function() replay_sub() end)
