...
--Changes are recommended to be made in the script-opts directory.
	...
	["all", "recents", "distinct", "protocols", "playing", "fileonly", "titleonly", "keywords", "playlists"]
...
local protocols = {'https?:', 'magnet:', 'rtmps?:', 'smb:', 'ftps?:', 'sftp:', 'edl:'}
local available_filters = {'all', 'recents', 'distinct', 'playing', 'protocols', 'fileonly', 'titleonly', 'timeonly', 'keywords', 'playlists'}
...
function get_file()
    ...
    if current_playlist and current_playlist ~= "" then
        path = current_playlist
        local state = ""
        local curr_pos = mp.get_property("playlist-pos-1")
        local total = mp.get_property("playlist-count")
        if curr_pos and total then state = " [" .. curr_pos .. "/" .. total .. "]" end
        title = "⫶☰ " .. title .. state
    else
...
function get_list_contents(filter, sort)
    ...
    if filter == 'playlists' then
		for i = 1, #list_contents do
			if contain_value({"⫶☰ "}, list_contents[i].found_line) then
				table.insert(filtered_table, list_contents[i])
			end
		end
	...
	if file_exists(list_contents[#list_contents - list_cursor + 1].found_path) or starts_protocol(protocols, list_contents[#list_contents - list_cursor + 1].found_path) then
		if not add_playlist then
			if filePath ~= list_contents[#list_contents - list_cursor + 1].found_path then
                ...
                if string.find(list_contents[#list_contents - list_cursor + 1].found_name, "⫶☰ ") then
                    playlist_pos = tonumber(string.match(list_contents[#list_contents - list_cursor + 1].found_name, "%[(%d+)%/%d+%]"))
                    if playlist_pos then
                        mp.add_timeout(0.2, function() mp.observe_property("playlist-count", "number", changeplpos) end) --нужно подождать, пока выгрузится старый плейлист
                    end
                end
                list_close_and_trash_collection()
			else
...
function get_list_keybinds()
    ...
    if string.find(string.upper(utils.to_string(o.list_select_keybind)), "MBTN_.*_DBL") then --баг https://github.com/mpv-player/mpv/issues/13083
        bind_keys({"MOUSE_MOVE"}, 'bugfix', function() end)
    end
...
function unbind_list_keys()
    ...
    if string.find(string.upper(utils.to_string(o.list_select_keybind)), "MBTN_.*_DBL") then
        unbind_keys({"MOUSE_MOVE"}, 'bugfix')
    end
    ...
    unbind_keys(o.list_search_activate_keybind, 'list-search-activate')
...
function handle_input(input)
	...
	or input.event == "down" then
    update_search_results(input.key_text)
	end
end
...
function unbind_search_keys()
    mp.remove_key_binding("handle_input")
...
mp.register_script_message("history-search", function() -- открыть историю просмотра сразу с полем для поиска
    if not list_drawn then display_list(o.open_list_keybind[1][2]) end
    list_search_activate()
end)

mp.register_script_message("current-playlist", function(v) current_playlist = v end) --получаем текущий открытый файл плейлиста от скрипта auto-save-state.lua из сборки
    -- на версии mpv 0.37 появилось свойство для получения файла плейлиста напрямую, однако я хочу сохранить совместимость с более шустрой версией 0.35.1 которой пользуюсь сам

function changeplpos(k, v)  -- восстановление номера серии при открытии плейлиста (на всякий случай - плеер со скриптом auto-save-state должен сам это запоминать)
	if v == nil or v < 2 then return end
    if playlist_pos == nil or mp.get_property_number("playlist-pos-1") == playlist_pos then
        mp.unobserve_property(changeplpos)
        return
    end
    msg.info("Playlist position changed to saved value: " .. playlist_pos)
