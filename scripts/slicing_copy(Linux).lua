...
local function parse_user_dirs_file()
    local home = os.getenv("HOME")
    if not home then return nil end
    local user_dirs_path = home .. "/.config/user-dirs.dirs"
    local file, err = io.open(user_dirs_path, "r")
    if not file then
        msg.warn("slicing_copy: Could not open " .. user_dirs_path .. ". Error: " .. (err or "unknown"))
        return nil
    end

    for line in file:lines() do
        if not line:find("^#") and line:find("XDG_VIDEOS_DIR") then
            local path = line:match('^XDG_VIDEOS_DIR%s*=%s*"(.*)"')
            if path then
                file:close()
                path = path:gsub("$HOME", home, 1)
                msg.info("slicing_copy: Found video directory in user-dirs.dirs: " .. path)
                return path .. "/"
            end
        end
    end
...
local function get_default_target_dir()
    -- Try XDG_VIDEOS_DIR environment variable first
    local videos_dir_env = os.getenv("XDG_VIDEOS_DIR")
    if videos_dir_env and videos_dir_env ~= "" then
        return videos_dir_env .. "/"
    end

    -- If not set, try to parse the user-dirs.dirs config file
    local videos_dir_file = parse_user_dirs_file()
    if videos_dir_file then
        return videos_dir_file
    end
    
    -- Fallback for Linux/macOS
    local home = os.getenv("HOME")
    if home and home ~= "" then
        msg.warn("slicing_copy: Falling back to default '~/Videos/' directory.")
        return home .. "/Videos/"
    end
...
local function cut(shift, endpos)
    ...
    local video_id = mp.get_property_number("current-tracks/video/id", 0)
    if video_id > 0 then
        cmds:arg("-map", string.format("0:v:%d?", video_id - 1))
    else
        cmds:arg("-vn") -- No video if no track is selected
    end

    if copy_audio then
        local audio_id = mp.get_property_number("current-tracks/audio/id", 0)
        if audio_id > 0 then
            cmds:arg("-map", string.format("0:a:%d?", audio_id - 1))
        else
            cmds:arg("-an")
        end
    else
        cmds:arg("-an") -- No audio if disabled by user
    end

    local sub_id = mp.get_property_number("current-tracks/sub/id", 0)
    if sub_id > 0 then
        cmds:arg("-map", string.format("0:s:%d?", sub_id - 1))
    else
        cmds:arg("-sn") -- No subtitles
    end
