...
function reload(path, time_pos)
  ...
  if time_pos then
	mp.add_timeout(0.3, function() mp.observe_property("time-pos", "native", restore) end)
  end
  
  if mp.get_property("path") ~= mp.get_property("stream-path") then --video was parsed by yt-dlp, external subs connect automatically
    return
  end
...
function reload_resume()
  ...
  if reload_duration and reload_duration > 0 then
    ...
    reload(path, timepos)
  -- VODs get stuck when reload is called without a timepos
...
function restore(k, v)
  if not mp.get_property("time-pos") or timepos == nil then return end
  if math.abs(timepos - mp.get_property("time-pos")) > 1 then
    mp.commandv("seek", timepos, "absolute", "exact")
  end
  mp.unobserve_property(restore)
end
