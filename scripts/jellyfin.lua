...
local function update_list()
	...
	for i = list_start[layer], list_start[layer] + magic_num do
		...
		local part_count = 1
		local base_name = ""
		if items[i].PartCount ~= nil then
			part_count = items[i].PartCount
		end
		items[i].PartCount = 1
...
local function update_image(item)
	...
	local filepath = ""
	if async[1] ~= nil then
		mp.abort_async_command(async[1])
	end
	...
	if item.ImageTags.Primary ~= nil then
		...
		async[1] = mp.command_native_async({
			...
			args = {
				"mpv",
				options.url .. "/Items/" .. item.Id .. "/Images/Primary?width=" .. width .. "&height=" .. height,
				"--no-config",
				"--msg-level=all=no",
				"--vf=lavfi=[format=bgra]",
				"--of=rawvideo",
				"--o=" .. filepath,
			},
		}, function(success, result, error)
			show_image(success, result, error, { width, height, filepath })
		end)
	end
end
...
local function update_overlay()
	...
	local base_url = options.url
		...
		.. "&parentId="
		.. parent_id[layer]
		.. "&enableImageTypes=Primary&imageTypeLimit=1&fields=PrimaryImageAspectRatio,Taglines,Overview,MediaSources"
	if layer == 2 then
...
local function width_change(name, data)
	...
	if shown then
		update_image(items[selection[layer]])
	end
end
...
local function play_video()
	...
	mp.commandv(
		"loadfile",
		options.url .. "/Videos/" .. id .. "/stream?static=true",
		"insert-at-play",
		selection[layer] - 1
	)
...
local function connect()
	local request = mp.command_native({
		...
		args = {
			...
			"-d",
			'{"username":"' .. options.username .. '","Pw":"' .. options.password .. '"}',
		},
	})
...
if options.show_by_default == "on" then
	toggle_overlay()
end
