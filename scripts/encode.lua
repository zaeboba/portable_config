...
function get_extension(path)
    ...
    if candidate then
        for _, ext in ipairs({ "webm", "mp4", "avi", "opus", "mp3" }) do
...
function set_timestamp(profile)
    ...
    if not start_timestamp or profile ~= profile_start then
        ...
        local msg = function()
            mp.osd_message(
                string.format("Trim mp4, mkv, webm, opus, mp3\nStart Time has been set\nESC clear Start Time\n\nF4 or ENTER sets End Time\nm mute exclude audio\nv exclude subs", profile or "default"),
                timer_duration
            )
        end
        msg()
        ...
        clear_timestamp()
        mp.osd_message(string.format("Trimming from %s to %s"
        ...
        local settings = {
            ...
            append_filter = "",
            codec = "-c:v copy -c:a copy -c:s copy -reset_timestamps 1",
            output_format = "$f_$n.$x",
            --Mac
            output_directory = "/Users/adam/Movies/mpvclips/video",
            -- Linux
            --output_directory = "/home/geektips/Videos/mpvclips/video",
            -- Windows
        --  output_directory = "C:\Users\geektips\Videos\mpvclips\video",
        ...
        if profile then
            ...
            if settings.container ~= "" then
                msg.warn("The 'container' setting is deprechated, use 'output_format' now")
